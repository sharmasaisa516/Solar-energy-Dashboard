...
page = st.sidebar.radio(
    "Go to:",
    ["Home", "Uses of Solar Energy", "Analysis of Solar Energy"]
)
...
if page == "Analysis of Solar Energy":
    ...
    st.write(
        ...
        "homes and industries. It is a clean, renewable, and eco-friendly "
        "source of energy."
    )
    ...
    with col3:
        ...
        ">
            ...
            <h2 style="color:#222222;">2026</h2>
            <p style="color:#222222;">
                CO₂ emissions reduced through solar energy
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.image(
            "1.png",
            caption="Solar Panels in Action",
            use_container_width=True
        )
    ...
    country_totals = (
        solar_df.groupby("COUNTRY", as_index=False)["CONSUMPTION"].sum()
    ...
    col3.metric("Peak Month", f"{peak_row['MONTH_NAME']} {int(peak_row['YEAR'])}")

    yearly = country_data.groupby("YEAR", as_index=False)["CONSUMPTION"].sum()
    fig_trend = px.line(
        yearly, x="YEAR", y="CONSUMPTION", markers=True,
